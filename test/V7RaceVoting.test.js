...
describe("V7 Race Voting Core", function () {
  ...
  it("allows only one secret pick per wallet", async function () {
    ...
    await expect(
      voting.connect(voter).commitVote(await voting.makeCommitment(2, salt2), [24])
  ...
  it("rejects Hall of Fame NFTs because they have 0 VP", async function () {
    ...
    await expect(
      voting.connect(owner).commitVote(await voting.makeCommitment(3, salt), [1])
    ).to.be.revertedWith("token not voting eligible");
  });
  ...
  it("prevents reuse after a voted NFT is transferred to another wallet", async function () {
    ...
    await expect(
      voting.connect(other).commitVote(await voting.makeCommitment(5, salt2), [23])
    ).to.be.revertedWith("token already used");
  });

  it("rejects a voting NFT while it is held in Team Wallet", async function () {
    ...
    await expect(
      voting.connect(team).commitVote(await voting.makeCommitment(6, salt), [23])
    ).to.be.revertedWith("token not voting eligible");
  });
