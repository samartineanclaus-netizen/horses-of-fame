...
describe('Readiness canonical local dry run',function(){this.timeout(180000);
 ...
 before(()=>{originalFetch=global.fetch;global.fetch=async(url,options)=>{
  ...
  if(url===new URL(require('../../public/genesis/unrevealed.json').image,'https://hof-fixture.example.org/genesis/unrevealed.json').href)return new Response(fs.readFileSync('public/brand/hof-logo.webp'),{headers:{'content-type':'image/webp'}});
