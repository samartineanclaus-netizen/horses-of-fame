...
describe("V7 locked race scoring", function () {
  it("uses exactly 25/18/15/12/10/8/6/4/2/1 for positions 1-10", async function () {
    ...
    for (let position = 1; position <= 10; position++) {
      expect(await voting.pointsForPosition(position)).to.equal(BigInt(expected[position - 1]));
    }
  });

  it("gives zero points to every position from 11 through 22", async function () {
    const { voting } = await deployFixture();
    for (let position = 11; position <= 22; position++) {
      expect(await voting.pointsForPosition(position)).to.equal(0n);
    }
  });
  ...
  it("maps ranking positions back to the correct HOF competitor", async function () {
    ...
    // With no revealed votes all competitors have 0 VP, so V7 tie-break
    // ranks HOF #1..#22 in numeric order.
    const points = await voting.horseRacePoints();
    const expected = [25, 18, 15, 12, 10, 8, 6, 4, 2, 1];
    for (let horse = 1; horse <= 10; horse++) {
      expect(points[horse - 1]).to.equal(BigInt(expected[horse - 1]));
    }
    for (let horse = 11; horse <= 22; horse++) {
      expect(points[horse - 1]).to.equal(0n);
    }
  });
