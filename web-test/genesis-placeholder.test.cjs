...
test('canonical metadata only has name, description and absolute canonical logo URL',async()=>{
 assert.deepEqual(Object.keys(metadata).sort(),['description','image','name']);assert.match(metadata.description,/TESTNET \/ TEST ONLY \/ NO VALUE/);assert.equal(metadata.image,'https://hof-site.vercel.app/brand/hof-logo.webp');
 const calls=[];const mock=fetcher();await validatePlaceholder(uri,46630,async(url,options)=>{calls.push(url);assert.equal(options.redirect,'error');return mock(url);});assert.deepEqual(calls,[uri,'https://hof-site.vercel.app/brand/hof-logo.webp']);
});
test('mint window is exactly seven days',()=>assert.equal(MINT_WINDOW_SECONDS,604800n));
for(const chain of [1,4663,42161]) test(`testnet metadata rejected on chain ${chain} before HTTP`,async()=>assert.rejects(validatePlaceholder(uri,chain,()=>{throw Error('unexpected HTTP');}),/TESTNET/));
