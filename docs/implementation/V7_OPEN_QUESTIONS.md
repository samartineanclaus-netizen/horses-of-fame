...
11. **Genesis mint ordering and ownership of the 22 HOF tokens.** The current testnet contract mints sequential token IDs, and its deterministic test mapping treats `#1–#22` as HOF. V7 fixes 2,000 Public + 111 Community + 111 Team Reserve but does not state which allocation initially receives the 22 HOF racers, nor the mainnet mint order needed before Public Mint starts. The deployment flow therefore must not silently pre-mint the 22 HOF tokens to a wallet until this is decided.
12. **Community season finalization scalability.** The current testnet implementation calculates Community Top 3 and archives/reset state by iterating the active scoring-wallet set at season finalization. At a large Chapter I participation count this may become too expensive for one EVM transaction. The final mainnet settlement architecture must preserve the exact V7 standings and approved tie-break semantics while using a gas-safe batching/proof/settlement design; no trusted off-chain winner rule or changed scoring rule should be introduced silently.
