...
class RaceRevealService {
 constructor({board,signer,relayer,treasuryAddress,directory,keyFor,raceAt,factoryAt,ingressToken,allowLocal=false,pollMs=1000,scoreBatch=25,buildResult=buildRelayedResult,lockDirectory=path.join(os.tmpdir(),'hof-relayer-locks')}){
 ...
 async verifyProposal(race,result){
  ...
  for(let from=0;from<=head.number;from+=2000)logs.push(...await race.queryFilter(race.filters.ResultProposed(),from,Math.min(from+1999,head.number)));
 ...
 async tick(){
  ...
  try{
   ...
   for(let i=0;i<count;i++){
    ...
    if(!result){result=await this.buildResult(race,this.signer,await this.key(race));const hash=keccak256(AbiCoder.defaultAbiCoder().encode(['bytes32','uint256[22]','bytes32'],[await race.frozenRecordsHash(),result.totals,result.root]));this.transactions.plan(race.target,hash);this.cache.set(race.target,result);}
