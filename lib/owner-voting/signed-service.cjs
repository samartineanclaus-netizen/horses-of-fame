...
class SignedVoteQueue {
 constructor({race,relayer,treasuryAddress,journal,batchSize=25,maxGas=5000000n,fromBlock=0,flushMs=3000,urgentMs=1000,deadlineWindow=60,receiptTimeoutMs=30000,maxAttempts=3,lockDirectory=path.join(os.tmpdir(),'hof-relayer-locks')}){
 ...
 async checkRelayer(){
  ...
  const binding=json({chain:String(chain),race:getAddress(this.race.target),relayer:address,treasury:this.treasuryAddress});
