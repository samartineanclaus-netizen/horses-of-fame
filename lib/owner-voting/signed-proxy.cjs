...
async function signedProxy(request,base,path,fetcher=fetch,ingressToken=process.env.HOF_SIGNED_INGRESS_TOKEN){
 const reply=(body,status)=>Response.json(body,{status,headers:{'Cache-Control':'no-store'}});
 if(!base||typeof ingressToken!=='string'||ingressToken.length<32)return reply({error:'Voting service unavailable'},503);
 ...
 try{
  ...
  const response=await fetcher(url,{method:'POST',body:Buffer.concat(chunks),headers:{'Content-Type':'application/json',Authorization:`Bearer ${ingressToken}`},redirect:'error',cache:'no-store',signal:AbortSignal.timeout(15000)});
