...
import {BOARD_ABI,GENESIS_ABI,raceAt,readRace,inventory,readLeaderboards,assertSnapshot} from '../../lib/owner-voting/website.cjs';
...
const phases = {scheduled:'Voting has not opened',open:'Secret voting is open',awaiting:'Voting closed · awaiting complete Race Reveal',finalized:'Race Reveal complete'};
...
export default function Voting({mode}) {
  ...
  useEffect(()=>{
    ...
    const reset=()=>{epoch.current++;setWallet('');setSnapshot(null);setExcludedTokens([]);setHorse(null);setMessage('Wallet changed. Connect again to vote.');};
  ...
  async function connect() {
    ...
    try {
      ...
      const signer=await provider.getSigner();setWallet(await signer.getAddress());setExcludedTokens([]);setMessage('');
  ...
  const canVote=phase==='open' && inv && !inv.excluded && validSelected && (inv.voted || horse) && !busy;
