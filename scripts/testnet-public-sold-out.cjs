...
async function makeAPI(provider,env,mode='dry-run'){
 ...
 async send(kind,amount,quote,nonce){if((kind==='fund'&&mode!=='fund')||(kind==='approve'&&mode!=='approve')||(kind==='mint'&&mode!=='mint'))throw Error('Write phase not enabled');const w=kind==='fund'?privateWallet(env.HOF_TEST_FUNDER_KEY_FILE,provider,funder):privateWallet(env.HOF_TEST_BUYER_KEY_FILE,provider,buyer);return w.sendTransaction({...api.payload(kind,amount),nonce,gasLimit:quote.gasLimit,gasPrice:quote.feeCap});},
...
async function main(){
 require('dotenv').config({quiet:true});const args=process.argv.slice(2);if(args.length>1||args.some(x=>!['--dry-run','--fund','--approve','--mint'].includes(x)))throw Error('Use --dry-run, --fund, --approve or --mint');
 const mode=args[0]==='--fund'?'fund':args[0]==='--approve'?'approve':args[0]==='--mint'?'mint':'dry-run';
