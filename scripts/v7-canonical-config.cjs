...
function address(env,key){const v=required(env,key);if(!isAddress(v)||getAddress(v)===ZeroAddress)throw Error(`Invalid ${key}`);return getAddress(v);}
...
function systemConfig(env){return {...roles(env),usdc:address(env,'HOF_USDC_ADDRESS'),audit:address(env,'AUDIT_WALLET'),deadline:timestamp(env,'MINT_DEADLINE_UNIX'),placeholder:required(env,'GENESIS_PLACEHOLDER_URI')};}
